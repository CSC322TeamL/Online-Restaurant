...
public class DeliveryFinishedFragment extends Fragment implements View.OnClickListener {
    private DeliveryActionInterface actionListener;
    private RecyclerView orderList;
    private DeliveryOrderListAdapter adapter;
    private FloatingActionButton btnRefresh;

    public DeliveryFinishedFragment( DeliveryActionInterface actionListener){
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        adapter = new DeliveryOrderListAdapter(DeliveryDataStore.TYPE_FINISHED, DeliveryDataStore.getInstance().getOrderFinished());
        adapter.setActionListener(actionListener);
        orderList.setAdapter(adapter);
        orderList.setLayoutManager(new LinearLayoutManager(getActivity()));

        if(null != actionListener){
            actionListener.onFinishedFragmentRefresh();
        }
    }

    public void notifyDataChange(){
        if(null != adapter){
            adapter.setOrderList(DeliveryDataStore.getInstance().getOrderFinished());
